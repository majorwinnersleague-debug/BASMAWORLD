...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey) {
    return NextResponse.json({ error: 'Upload-Post not configured' }, { status: 503 })
  }
  ...
  try {
    ...
    const jwtRes = await fetch('https://api.upload-post.com/api/uploadposts/users/generate-jwt', {
      ...
      body: JSON.stringify({
        ...
        redirect_url: `https://basmaworld.com/social-media/onboarding?session_id=${sessionId}&connected=true`,
        logo_image: 'https://basmaworld.com/images/basma-hero.jpg',
        connect_title: 'Connect Your Social Accounts',
        connect_description: 'Link the platforms where you want your content posted automatically.',
        show_calendar: true,
    ...
    if (!jwtRes.ok || !jwtData.access_url) {
      ...
      return NextResponse.json({ error: 'Could not generate connection URL' }, { status: 500 })
    }
