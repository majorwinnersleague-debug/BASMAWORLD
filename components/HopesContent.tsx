...
export default function HopesContent() {
  ...
  async function handleSelect(value: string, label: string) {
    ...
    try {
      ...
      if (data.resources?.length) {
        ...
        posthog.capture('hopes_chance_search_no_results', { category: value, category_label: label })
      }
    } catch {
      setError('Unable to load resources. Please call 211 for immediate assistance.')
      posthog.capture('hopes_chance_search_error', { category: value, category_label: label })
    } finally {
      setLoading(false)
    }
  }
